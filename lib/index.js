...
const createReporter = () => {
  ...
  const println = (input, indentLevel) => {
    input = input || ''
  ...
  const handleAssertFailure = assert => {
    ...
    const writeDiff = opts => {
      ...
      return opts.value.replace(/(^\s*)(.*)/g, (m, one, two) => one + style(two))
    };
    ...
    let at = assert.diag.at
    let actual = assert.diag.actual
    let expected = assert.diag.expected
    ...
    if (expected_type === 'object') {
      ...
      const compared = diff.diffJson(actual, expected)
        .map(writeDiff)
        .join('');
